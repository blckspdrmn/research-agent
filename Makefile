...
.PHONY: help build up down restart-api api-logs ps migrate revision seed test-db lint format test ci ci-frontend shell-api psql down-clean npm-install
...
ci: lint test ci-frontend ## CIと同じ検査をまとめて実行
